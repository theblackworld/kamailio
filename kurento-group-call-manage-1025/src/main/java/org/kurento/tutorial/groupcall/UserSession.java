...
public class UserSession implements Closeable {
  ...
  public void receiveVideoFrom(UserSession sender, String sdpOffer) throws IOException {
    ...
    final String ipSdpAnswer = this.getEndpointForUser(sender).processOffer(sdpOffer);//接收端返回sdpOffer，此时接收端准备好接受媒体流
    ...
    /*****************************************************/
/*  public void roomNotExist() throws IOException{
    log.info("room is null2");
    final JsonObject scParams1 = new JsonObject();
    scParams1.addProperty("id", "roomNotExist");
    this.sendMessage(scParams1);
  }*/
     /*****************************************************/
    public void closedDyServer() throws IOException{
    log.info("closedDyServer");
    JsonObject scParams1 = new JsonObject();
    scParams1.addProperty("id", "closedDyServer");
    this.sendMessage(scParams1);
  }
    ...
    if(null!=incoming){
        incoming.release(new Continuation<Void>() {
          ...
          public void onError(Throwable cause) throws Exception {
            log.info("PARTICIPANT {}: Could not release incoming EP for {}", UserSession.this.name,
                senderName);
          }
        });
    }
  ...
  public void close() throws IOException {
    ...
    outgoingMedia.release(new Continuation<Void>() {
      ...
      @Override
      public void onError(Throwable cause) throws Exception {
        log.info("USER {}: Could not release outgoing EP", UserSession.this.name);
      }
    });
  }
