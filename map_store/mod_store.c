...
int room_entry_add(struct sip_msg* _m,char* _roomid, char* _callid, char* _fromtag, char* _totag){	  
  ...
  if(!(entry = (struct map_entry*) shm_malloc (sizeof (struct map_entry)))){
...
void print_by_room(const char* s){	
  ...
  for(int i=0; i<start->size; i++){
	LM_INFO("the callid is:%s,the fromtag is:%s, the totag is: %s\n",start->entry[i]->callid,start->entry[i]->formtag,start->entry[i]->totag);  
  }
}
/*******************************************************************************
 * 名称: print_all
  ...
  for(i=0; i<hashsize; i++){	  
    ...
    while(room != NULL){
      ...
      for(int j=0; j<room->size; j++){ 
		 LM_INFO("the callid is:%s,the fromtag is:%s, the totag is: %s\n",start->entry[j]->callid,start->entry[j]->formtag,start->entry[j]->totag);
      }
      room = room->next;
    }
  }
