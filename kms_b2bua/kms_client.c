/*
   back 2 back entities module forked from opensips
   and modify it to adapt to kamailio   
*/
#include <stdio.h>
#include <stdlib.h>
#include <utime.h>
...
// this function in kms_dlg.c and may have problem because these does not exists
// uac_auth;
...
str* client_new(client_info_t* ci, b2b_notify_t b2b_cback, b2b_add_dlginfo_t add_dlginfo, str* param){
  ...
  if(ci->from_tag) from_tag = *ci->from_tag;
  else generate_tag(&from_tag, &ci->from_uri, ci->extra_headers);

  /* create a dummy b2b dialog structure to be inserted in the hash table*/
  size = sizeof(b2b_dlg_t) + ci->to_uri.len + ci->from_uri.len + ci->from_dname.len + ci->to_dname.len +
  ...
  if(ci->to_dname.s)  CONT_COPY(dlg, dlg->to_dname, ci->to_dname);
  if(ci->from_dname.s) CONT_COPY(dlg, dlg->from_dname, ci->from_dname);
  ...
  if(b2breq_complete_ehdr(ci->extra_headers, ci->client_headers, &ehdr, ci->body, &ci->local_contact)< 0){
  ...
  if(ci->req_uri.s) td.rem_target = ci->req_uri;
  else td.rem_target = ci->to_uri;
  ...
  if(ci->dst_uri.len) td.obp = ci->dst_uri;
  ...
  if(td.route_set) pkg_free(td.route_set);
...
dlg_t* b2b_client_build_dlg(b2b_dlg_t* dlg, dlg_leg_t* leg){
  ...
  if(leg){
	if(leg->route_set.s && leg->route_set.len){
	  if(parse_rr_body(leg->route_set.s, leg->route_set.len, &td->route_set)< 0){
  ...
  if(dlg->send_sock)
	LM_DBG("send sock= %.*s\n", dlg->send_sock->address_str.len, dlg->send_sock->address_str.s);
