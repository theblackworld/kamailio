 ...
 *
 *  forked from opensips b2bua_entities and modify it to adapt to kamailio;
 *
 *  and add some function into such entities;
 * 
 * 
*/
...
//src 标定server or client;
typedef int (*b2b_add_dlginfo_t)(str* key, str* entity_key, int src, b2b_dlginfo_t* info);

/*
   Dialog state ;
   
   opensips标定了如下的几种状态，后面如有需要可以添加更多信息!!
*/
...
/*
   ...
   b2bua主要用到的数据结构,对各项属性进行详细说明;
   
   sip routing:
   
      route set > contact > from_uri; 
*/
typedef struct b2b_dlg
{
	...
	str                  to_uri;
	str                  to_dname;
	str                  tag[2];          // from tag, to tag;   
	unsigned int         cseq[2];         // 近端与远端事物序列号;
	unsigned int         last_invite_cseq;
	
	str                  route_set[2];    // record route;
	str                  contact[2]; 
...
/*
  hash table with lock;
*/
typedef struct b2b_entry 
{
	b2b_dlg_t* first;
	gen_lock_t lock;   // why not only one lock;
	int checked;
}b2b_entry_t;
...
typedef str* (*b2b_server_new_t) (struct sip_msg* , str* local_contact, b2b_notify_t , str* param);
