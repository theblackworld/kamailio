 ...
 * 功能:  根据key获取相应的hash_index && local_index;

 * 形参: 
		 
 * 返回:  -1 can not get the index or get the index;

 * 说明:  key format : B2B.hash_index.local_index.timestamp;
 ******************************************************************************/
 ...
 * 功能:  根据hash_index && local_index 生成key

 * 形参: 
		 
 * 返回:  NULL or str* ( generate the key );
 ...
 * 功能:  根据在共享内存中复制一份dlg信息;

 * 形参:   
		 
 * 返回:  null or the new_dlg in shared memory;
...
b2b_dlg_t* b2b_dlg_copy(b2b_dlg_t* dlg){
  ...
  if(dlg->ruri.s) CONT_COPY(new_dlg, new_dlg->ruri, dlg->ruri);
  ...
  if(dlg->tag[0].len && dlg->tag[0].s) CONT_COPY(new_dlg, new_dlg->tag[0], dlg->tag[0]);
  if(dlg->tag[1].len && dlg->tag[1].s) CONT_COPY(new_dlg, new_dlg->tag[1], dlg->tag[1]);
  if(dlg->route_set[0].len && dlg->route_set[0].s) CONT_COPY(new_dlg, new_dlg->route_set[0], dlg->route_set[0]);
  if(dlg->route_set[1].len && dlg->route_set[1].s) CONT_COPY(new_dlg, new_dlg->route_set[1], dlg->route_set[1]);
  if(dlg->contact[0].len && dlg->contact[0].s) CONT_COPY(new_dlg, new_dlg->contact[0], dlg->contact[0]);
  if(dlg->contact[1].len && dlg->contact[1].s) CONT_COPY(new_dlg, new_dlg->contact[1], dlg->contact[1]);
  ...
  if(dlg->from_dname.s) CONT_COPY(new_dlg, new_dlg->from_dname, dlg->from_dname);
  if(dlg->to_dname.s)  CONT_COPY(new_dlg, new_dlg->to_dname, dlg->to_dname);
...
/*
   ...
   no need to care about memory leak???????????????????????
...
void b2b_delete_record(b2b_dlg_t* dlg, b2b_table htable, unsigned int hash_index){
  ...
  if(dlg->prev == NULL)  htable[hash_index].first = dlg->next;

  else 	dlg->prev->next = dlg->next;
 
  if(dlg->next) dlg->next->prev = dlg->prev;

  if(htable == server_htable && dlg->tag[CALLEE_LEG].s) shm_free(dlg->tag[CALLEE_LEG].s);

  b2b_delete_legs(&dlg->legs);

  if(dlg->uac_tran) tmb.unref_cell(dlg->uac_tran);

  if(dlg->uas_tran) tmb.unref_cell(dlg->uas_tran);

  if(dlg->ack_sdp.s) shm_free(dlg->ack_sdp.s);

  shm_free(dlg);
}

void b2b_entity_delete(enum b2b_entity_type et, str* b2b_key,  b2b_dlginfo_t* dlginfo, int db_del){
  ...
  if(db_del) b2b_entity_db_delete(et, dlg);
...
*    export function
       ...
       used for other file to call it;
*/
 ...
 * 功能: 根据hash_index and local index 查找指定的dialog信息; 

 * 形参: 
		 
 * 返回: NULL or dialog info;

 * 说明: /* this is only called by server new 
 ******************************************************************************/
...
str* b2b_htable_insert(b2b_table table, b2b_dlg_t* dlg, int hash_index, int src, int reload){
  ...
  if(it == NULL)  table[hash_index].first = dlg;
  else{
	while(it){
	  prev_it = it;
	  it = it->next;
  ...
  if(b2b_key == NULL){
	if(!reload) lock_release(&table[hash_index].lock);
  ...
  if(src == B2B_SERVER){
	dlg->tag[CALLEE_LEG].s = (char*)shm_malloc(b2b_key->len);
	  ...
	  if(!reload && b2be_db_mode == WRITE_THROUGH) b2be_db_insert(dlg, src);
	}
...
void destroy_b2b_htables(void){
  ...
  if(server_htable){
	for(i= 0; i< server_hsize; i++){
	  ...
	  while(dlg){
		aux = dlg->next;
		if(dlg->tag[CALLEE_LEG].s) shm_free(dlg->tag[CALLEE_LEG].s);
		if(dlg->ack_sdp.s) shm_free(dlg->ack_sdp.s);
		shm_free(dlg);
		dlg = aux;
	  }
 ...
 * 说明:   this is called by server new; 
******************************************************************************/
...
int b2b_send_reply(b2b_rpl_data_t* rpl_data)
{
    ...
    //通过hash index and local_index 获得指定的dialog;
	lock_get(&table[hash_index].lock);
...
*/
	...
	else
	{
		...
		{
			//获得事物中的method;            
			if(parse_method(tm_tran->method.s, tm_tran->method.s + tm_tran->method.len, &method_value) == 0)
			{
	...
	// 如果不是uas tran and update tran;
	// uac本身是不具备send reply的功能;
	...
	if(method_value==METHOD_INVITE && (dlg->state==B2B_CONFIRMED || dlg->state==B2B_ESTABLISHED))
	{
	...
	if(code >= 200)
	{
		if(method_value==METHOD_INVITE)
		{
			if(code < 300)
				...
				dlg->state= B2B_TERMINATED;
			UPDATE_DBFLAG(dlg);  //往mysql中插入当前dialog信息;
		}
	...
	if(method_value == METHOD_BYE)
		dlg->state = B2B_TERMINATED; 
	//terminated and then delete the record;
...
int b2b_send_indlg_req(b2b_dlg_t* dlg, enum b2b_entity_type et, str* b2b_key, str* method, str* ehdr, str* body, unsigned int no_cb){
  ...
  td = build_dlg(dlg);
  if(td == NULL){
	LM_ERR("Failed to build tm dialog structure, was asked to send [%.*s]" " request\n", method->len, method->s);
	if(b2b_key_shm)
  ...
  else if(method_value == METHOD_INVITE){
...
error:
  if(td) free_tm_dlg(td);
  if(b2b_key_shm) shm_free(b2b_key_shm);
 ...
 * 功能:   dialog中发送request请求;

 * 形参:   req_data :  et       :       server, client;
		               b2b_key  :       identify the dialog;
                       method   :       method for request;
					   extra_headers :  extra headers to be included int the request;
					   body     :       body to be included in the request;
					   dlg_info :       extra params for matching the dialig
                       no_cb    :		callback request or not;			   
 * 返回:    -1 fail or 0 success  // may be need more debug information;  
...
int b2b_send_request(b2b_req_data_t* req_data){
  ...
  if(dlg->state < B2B_CONFIRMED){
	...
	else if(method_value!=METHOD_UPDATE && method_value!=METHOD_PRACK && method_value!=METHOD_CANCEL && (method_value!=METHOD_INVITE)){
  ...
  //??????established 阶段为什么可以继续发送消息 
  else if(dlg->state==B2B_CONFIRMED && method_value!=METHOD_ACK && dlg->last_method == METHOD_INVITE){
  ...
  if(b2be_db_mode==WRITE_THROUGH && current_dlg!=dlg && dlg->state>B2B_CONFIRMED) {
	if(b2be_db_update(dlg, et) < 0) LM_ERR("Failed to update in database\n");
  }
...
dlg_leg_t* b2b_find_leg(b2b_dlg_t* dlg, str to_tag){
  ...
  while(leg){
	/* compare the tag */
	if(to_tag.len == leg->tag.len && strncmp(to_tag.s, leg->tag.s, to_tag.len)==0){
...
dlg_leg_t* b2b_new_leg(struct sip_msg* msg, str* to_tag, int mem_type){
  ...
  if(mem_type == SHM_MEM_TYPE) new_leg = (dlg_leg_t*)shm_malloc(size);
  else new_leg = (dlg_leg_t*)pkg_malloc(size);
...
int b2b_send_req(b2b_dlg_t* dlg, enum b2b_entity_type etype, dlg_leg_t* leg, str* method, str* extra_headers, str* body){
 
  dlg_t* td;
  int result;

  if(!dlg->callid.s || !dlg->callid.len) return -1;

  //if(!dlg->callid.s || !dlg->callid.len) return -1;

  LM_DBG("start type=%d\n", etype);
  
  if(etype == B2B_SERVER) td = b2b_server_build_dlg(dlg);
  else td = b2b_client_build_dlg(dlg, leg);
  ...
  if(method->len == ACK_LEN && strncmp(method->s, ACK, ACK_LEN)==0){
	td->loc_seq.value = dlg->last_invite_cseq;
	if(etype == B2B_SERVER) dlg->cseq[CALLEE_LEG]--;
	else dlg->cseq[CALLER_LEG]--;
  }
...
void b2b_tm_cback(struct cell *t, b2b_table htable, struct tmcb_params *ps){
  ...
  //parse the hash index and  local index;
  if(b2b_parse_key(b2b_key, &hash_index, &local_index)< 0){
  ...
  if(callid.s) dlg_based_search = 1;

  lock_get(&htable[hash_index].lock);
  
  // 根据dialog_info查找相应的dialog信息;
  if(!dlg_based_search){
  ...
  while (dlg && method_id==METHOD_INVITE && dlg->uac_tran != t) {
	...
	"transaction [%p]\n", dlg, dlg->uac_tran, t);
	if(dlg_based_search) 
	  dlg = b2b_search_htable_next_dlg( previous_dlg, htable, hash_index, local_index, &from_tag, &to_tag, &callid);
	else
	...
	if(dlg){
	  ...
	  if(statuscode>=200 && statuscode<300 && (dlg->state==B2B_ESTABLISHED || dlg->state==B2B_TERMINATED)){
		...
		if(dlg->callid.s==0 || dlg->callid.len==0) dlg->callid = msg->callid->body;
		if(b2b_send_req(dlg, etype, leg, &ack, 0, (dlg->ack_sdp.s?&dlg->ack_sdp:0)) < 0){
  ...
  if(statuscode >= 300){
	...
	if(method_id != METHOD_INVITE){
	  ...
	  dlg_state = dlg->state;
	  lock_release(&htable[hash_index].lock);
	  if(msg == FAKED_REPLY)  goto dummy_reply;
	  goto done1;
	}
	...
	if(dlg->state >= B2B_CONFIRMED){
	  ...
	  switch(statuscode){
		case 401:
		  if (0 == parse_www_authenticate_header(msg)) auth = get_www_authenticate(msg);
		  break;
	    case 407:
		  if (0 == parse_proxy_authenticate_header(msg)) auth = get_proxy_authenticate(msg);
		  break;
	  }
	  if(uac_auth_loaded && auth && dlg->state == B2B_NEW){
		...
		if(crd){
		  ...
		  uac_auth_api._do_uac_auth(&t->method, &t->uac[0].uri, crd, auth, &auth_nc_cnonce, response);
		  new_hdr = uac_auth_api._build_authorization_hdr(statuscode, &t->uac[0].uri, crd, auth, &auth_nc_cnonce, response);
		  if(!new_hdr){
		  ...
		  if (build_extra_headers_from_msg(t->uac[0].request.buffer,new_hdr, &extra_headers, &body) < 0 ){
		  ...
		  b2b_send_indlg_req(dlg, B2B_CLIENT, b2b_key, &t->method, &extra_headers, &body, 0);
